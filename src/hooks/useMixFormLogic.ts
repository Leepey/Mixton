...
export const useMixFormLogic = ({
  ...
  const handleMix = useCallback(async () => {
    ...
    setIsMixing(true);
    try {
      // Передаем числовое значение delayHours вместо строки ID пула
      await mixTons(
        parseFloat(amount), 
        note, 
        undefined, // recipient
        delayHours // передаем число, а не строку
      );
      setNote('');
      setAmount('');
      onMixSuccess?.();
  ...
  }, [amount, note, delayHours, validateAmount, mixTons, onMixSuccess]);
