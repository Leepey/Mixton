...
it('should process withdrawal queue correctly', async () => {
    ...
    // Дополнительные тесты для проверки очереди
...
it('should handle batch processing efficiently', async () => {
    ...
    for (let i = 0; i < 5; i++) {
        const lastDepositId = await mixton.getLastDepositId() - BigInt(4 - i);
        await mixton.sendWithdraw(
            admin.getSender(),
            recipient.address,
            toNano('0.5'),
    ...
    // Увеличиваем время, чтобы все элементы очереди были готовы к обработке
